...
class PolicyNetwork(nn.Module):
    ...
    def clip_actions(self, action):
        """Clip actions to the valid range for continuous action spaces."""

        if not self.is_continuous:
            return action  # No clipping needed for discrete actions    
    ...
    def get_parameters_count(self):
        """Utility function to count total parameters in the network."""
        return sum(p.numel() for p in self.parameters() if p.requires_grad) 
    
    def print_network_info(self):
        """Print detailed network information."""
        total_params = self.get_parameters_count()
        action_type = "Continuous" if self.is_continuous else "Discrete"
        print(
            f"Policy Network Info:\n"
